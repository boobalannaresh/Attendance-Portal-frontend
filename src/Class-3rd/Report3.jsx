...
function Report3() {
    ...
    const formik = useFormik({
      ...
      onSubmit: async (values) => {
        await axios.put(`${env.api}/class-3rd/${params.id}`, values, {
          headers: {                                                              // Changes Here
            "authorization": window.localStorage.getItem("app-token")
          }
        })
        alert("Update the Report Successfully")
        navigate("/portal/class-3rd/attendance")
      }
    });
    ...
    let loadUser = async () => {
      try {
        let user = await axios.get(`${env.api}/class-3rd/${params.id}`, {
          headers: {                                                          // Changes Here
            "authorization": window.localStorage.getItem("app-token")
          }
        });
      ...
      <div className="container mt-5 ">
        <form onSubmit={formik.handleSubmit}>
          ...
          <div className=' shadow-lg p-3 bg-body rounded btn btn-sm btn-warning mr-2'>
              <Link to="/portal/class-3rd/attendance" className="d-none d-sm-inline-block btn btn-sm btn shadow-lg"><h6>Back</h6></Link>
            </div>
